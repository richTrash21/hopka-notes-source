...
class ControlsSubState extends MusicBeatSubstate
{
	...
	function createTexts()
	{
		...
		for (i in 0...options.length)
		{
			...
			if (!option.isEmpty)
			{
				...
				final text:Alphabet = new Alphabet(200, 300, option.displayName, !isDisplayKey);
	...
	function playstationCheck(alpha:Alphabet)
	{
		...
		final gamepad = FlxG.gamepads.firstActive;
		final model = gamepad == null ? UNKNOWN : gamepad.detectedModel;
	...
	override function update(elapsed:Float)
	{
		...
		if (!binding)
		{
			...
			if (FlxG.keys.justPressed.ENTER || FlxG.gamepads.anyJustPressed(START) || FlxG.gamepads.anyJustPressed(A))
			{
				...
				else
				{
					...
					FlxTween.num(0, 0.6, 0.5, {ease: FlxEase.linear}, (a) -> bindingBlack.alpha = a);
					add(bindingBlack);

					bindingText = new Alphabet(FlxG.width / 2, 160, "Rebinding " + curOption.rebindName, false);
		...
		else
		{
			...
			else
			{
				...
				final curKeys:Array<FlxKey> = ClientPrefs.keyBinds.get(curOption.saveKey);
				final curButtons:Array<FlxGamepadInputID> = ClientPrefs.gamepadBinds.get(curOption.saveKey);
