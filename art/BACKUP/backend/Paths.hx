...
class Paths
{
	...
	static final INVALID_REGEX = ~/[~&\\;:<>#]/;
	static final HIDE_REGEX = ~/[.,'"%?!]/;

	static public var currentLevel(default, set):String;
	// define the locally tracked assets
	public static final localTrackedAssets = new Array<String>();
	public static final currentTrackedAssets = new Map<String, FlxGraphic>();
	public static final currentTrackedSounds = new Map<String, Sound>();
	public static final dumpExclusions = [
	...
	public static function clearUnusedMemory()
	{
		...
		for (key => obj in currentTrackedAssets)
		{
			...
			if (localTrackedAssets.contains(key) || dumpExclusions.contains(key) || obj == null)
				continue;
	...
	{
		return HIDE_REGEX.split(INVALID_REGEX.split(path.replace(" ", "-")).join("-")).join("").toLowerCase();
	}
	...
	public static function returnSound(path:String, key:String, ?library:String)
	{
		try
		{
			...
			filePath = getPath('$path/$key.$SOUND_EXT', SOUND, library);
			...
			#else
			{
				...
				currentTrackedSounds.set(filePath, OpenFlAssets.getSound(folder + getPath('$path/$key.$SOUND_EXT', SOUND, library)));
			}
			#end
			// localTrackedAssets.push(filePath);
			tryPush(localTrackedAssets, filePath);
			return currentTrackedSounds.get(filePath);
		}
