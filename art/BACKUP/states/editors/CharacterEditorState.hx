...
class CharacterEditorState extends backend.MusicBeatUIState
{
	...
	override function create()
	{
		...
		mouseManager.add(leHealthIcon,
			...
			function(icon:HealthIcon)
				icon.scale.set(icon.baseScale, icon.baseScale)
		);
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.justReleased && moveChar) // mouse up event
		{
			...
			if (moveOffset && char.animation.curAnim != null)
			{
				...
				ghostChar.addOffset(charAnim, ghostChar.offset.x, ghostChar.offset.y);
				genBoyOffsets();
			}
			else
			{
		...
		if (FlxG.mouse.justMoved)
		{
			if (moveChar)
			{
				if (moveOffset && char.animation.curAnim != null)
				{
					char.offset.x = char.animationsArray[curAnim].offsets[0] -= FlxG.mouse.deltaScreenX;
					char.offset.y = char.animationsArray[curAnim].offsets[1] -= FlxG.mouse.deltaScreenY;
					if (ghostChar != null)
					{
						ghostChar.animationsArray[curAnim].offsets[0] = char.offset.x;
						ghostChar.animationsArray[curAnim].offsets[1] = char.offset.y;
						if (char.animation.curAnim.name == (ghostChar.animation.curAnim?.name ?? ''))
						{
							ghostChar.offset.x = char.offset.x;
							ghostChar.offset.y = char.offset.y;
						}
					}
				}
				else
				{
					char.positionArray[0] = char.x += FlxG.mouse.deltaScreenX;
					char.positionArray[1] = char.y += FlxG.mouse.deltaScreenY;
					updatePointerPos();
				}
			}
